 ...
 * stdout/stderr/exit status/termination, and archive consumption). It is
 * not evidence for or against whether NAVISOMA needs its own
 * backend-neutral semantic core -- that is a separate question tracked in
 * Issue #13, out of scope for this file.
 *
 ...
 * actually gone. Used to prove an image handed to WSLC by a path other
 * than pull (tag, import, load) is actually runnable.
 */
